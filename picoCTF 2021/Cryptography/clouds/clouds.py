...
""")
	...
	if ui == "1":
		if len(notes) >= MAX_NOTES:
			print("Max capacity")
			return notes
		ui = input("Give me a note to encrypt: ").rstrip().encode()
		try:
		...
		ui = input("What index would you like to read? ").rstrip().encode()
		try:
			index = int(ui)
		except:
			print("Invalid index.")
...
flag = open("./flag").read().rstrip().encode()
notes = [encrypt(flag)]
notes = menu(notes)
while notes:
	notes = menu(notes)
