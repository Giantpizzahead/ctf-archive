...
bool get_combined_key(unsigned char* full_key, unsigned char* IV){
	...
	// Final key and IV are both 128 bits
	success = TRUE;
...
/*
Encrypts in blocks
Every character in each block is xored with a 16-byte block, initially the IV, then the previous encrypted block
