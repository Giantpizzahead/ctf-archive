...
def main():
	...
	#length of desired input is 70 as found from reversing the binary in ghidra
	...
	p = angr.Project('./darryl_vault', main_opts={'base_addr': base_addr}, auto_load_libs=True)
	...
	#extra input to set the code up, then find the flag!
	flag = claripy.Concat( claripy.BVV(b'4'), claripy.BVV(b'\n'))#, *flag_chars, claripy.BVV(b'\n'))

	# enable unicorn engine for fast efficient solving
	st = p.factory.full_init_state(
	        args=['./darryl_vault'],
	        add_options=set.union(angr.options.unicorn, {angr.options.ZERO_FILL_UNCONSTRAINED_MEMORY, angr.options.ZERO_FILL_UNCONSTRAINED_REGISTERS}),
	        stdin=flag
	       )
    ...
    for x in sm.deadended:
        if b"here you go" in x.posix.dumps(1):
            y.append(x)
